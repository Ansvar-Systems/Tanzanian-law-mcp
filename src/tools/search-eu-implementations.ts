/**
 * search_eu_implementations — Search EU directives/regulations that have Tanzanian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_tanzanian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  tanzanian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as tanzanian_statute_count
  ...
  if (input.has_tanzanian_implementation) {
    sql += ' HAVING tanzanian_statute_count > 0';
  }
