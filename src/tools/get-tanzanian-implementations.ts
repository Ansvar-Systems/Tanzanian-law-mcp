/**
 * get_tanzanian_implementations — Find Tanzanian statutes implementing a specific EU directive/regulation
...
export interface GetTanzanianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface TanzanianImplementationResult {
  document_id: string;
...
export async function getTanzanianImplementations(
  db: InstanceType<typeof Database>,
  input: GetTanzanianImplementationsInput,
): Promise<ToolResponse<TanzanianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as TanzanianImplementationResult[];
