#!/usr/bin/env tsx
/**
 * Database builder for Tanzania Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Tanzania Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Tanzanian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'TZ');
    insertMeta.run('source', 'tanzlii.org');
