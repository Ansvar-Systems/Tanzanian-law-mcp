#!/usr/bin/env tsx
/**
 * Tanzania Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from tanzlii.org (Akoma Ntoso HTML).
 *
 * Features:
 ...
 *
 * Data source: tanzlii.org (National Council for Law Reporting)
...
import { parseTanzaniaLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Tanzania Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('====================================================\n');
  console.log(`  Source: tanzlii.org (National Council for Law Reporting)`);
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseTanzaniaLawHtml(html, act);
  ...
  console.log(`\n  Source:      tanzlii.org (Akoma Ntoso HTML)`);
