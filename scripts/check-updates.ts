#!/usr/bin/env tsx
/**
 * Tanzanian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://tanzlii.org';
const PORTAL_NAME = 'Tanzania Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/tanzanian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Tanzanian Law MCP — Data Freshness Check');
