#!/usr/bin/env tsx
/**
 * Drift detection for Tanzania Law MCP.
 *
 * Checks if upstream Tanzania Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Tanzania Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'tanzanian-law-mcp/1.0 drift-detect' },
      });
